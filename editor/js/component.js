...
var Trigger = {
    ...
    CAST                 : { name: 'Cast',                 container: true, construct: TriggerCast     , premium: true            },
   // Click                : { name: 'Click',                 container: true, construct: TriggerCast       ,jun1: true        },
    ...
    TOOK_PHYSICAL_DAMAGE : { name: 'Took Physical Damage', container: true, construct: TriggerTookPhysicalDamage   , premium: true},
    TOOK_SKILL_DAMAGE    : { name: 'Took Skill Damage',    container: true, construct: TriggerTookSkillDamage    ,  premium: true },
    JUN_CAST_MONITOR    : { name: 'Jun Cast Monitor',    container: true, construct: JUN_CAST_MONITOR    ,  jun1: true },
    JUN_CLICK_MONITOR :  { name: 'Jun Click Monitor',            container: false,  construct: JUN_CLICK_MONITOR , jun1: true }
 ...
 */
var Target = {
	//Jun_Ergodic:    { name: 'Jun_Ergodic',                container: true,  construct: LineClientEffect         , jun1: true      },
//	Jun_LookAt:    { name: 'Jun_LookAt',                container: true,  construct: LineClientEffect         , jun1: true      },
Jun_Indicator: {name:"Jun_Indicator",             container: false,  construct: Jun_Indicator,      jun0: true     },
Jun_MM_Target:    { name: 'Jun_MM_Target',                container: false,  construct: Jun_MM_Target         , mm: true      },
    AREA     : { name: 'Area',     container: true, construct: TargetArea ,   premium: true    },
    CONE     : { name: 'Cone',     container: true, construct: TargetCone  ,   premium: true   },
    ...
    OFFSET   : { name: 'Offset',   container: true, construct: TargetOffset,   premium: true },
    OFFSETPLUS   : { name: 'Jun_OffsetPlus',   container: true, construct: Jun_OffsetPlus, jun1: true  },
    REMEMBER : { name: 'Remember', container: true, construct: TargetRemember,   premium: true },
    SELF     : { name: 'Self',     container: true, construct: TargetSelf    ,   premium: true },
    Single     : { name: 'Single',     container: true, construct: TargetSingle    ,   premium: true }
...
var Condition = {
    ...
    FLAG:        { name: 'Flag',        container: true, construct: ConditionFlag   ,   premium: true    },
    ...
    VALUE:       { name: 'Value',       container: true, construct: ConditionValue ,   premium: true     },
    Jun_ValueString:       { name: 'Jun_ValueString',       container: true, construct: Jun_ValueString ,   jun1: true     },
...
var Mechanic = {
    ...
    ITEM_PROJECTILE:     { name: 'Item Projectile',     container: false,  construct: MechanicItemProjectile},
    ...
    PARTICLE_ANIMATION:  { name: 'Particle Animation',  container: false, construct: MechanicParticleAnimation,   otf3: true},
    Jun_ParticleAnimation:  { name: 'Jun_ParticleAnimation',  container: false, construct: Jun_ParticleAnimation,   jun0:true},
    PARTICLE_EFFECT:     { name: 'Particle Effect',     container: false, construct: MechanicParticleEffect ,otf3:true},
    Jun_ProjectLie:     { name: 'Jun_ProjectLie',     container: false, construct: Jun_ProjectLie ,jun0:true},
    ...
    PROJECTILE:          { name: 'Projectile',          container: false,  construct: MechanicProjectile     },
    ...
    TRIGGER:             { name: 'Trigger',             container: false,  construct: MechanicTrigger,      premium: true      },
    ...
    VALUE_SET:           { name: 'Value Set',           container: false, construct: MechanicValueSet     ,   premium: true    },
    ...
    Jun_CircleParticle:  { name: 'Jun_CircleParticle',                container: false,  construct: Jun_CircleParticle         , jun0: true      },
    Jun_LineParticle:    { name: 'Jun_LineParticle',                container: false,  construct: Jun_LineParticle         , jun0: true      },
    Jun_Quake:    { name: 'Jun_Quake',                container: true,  construct: Jun_Quake         , jun0: true      },
    Jun_Mist:    { name: 'Jun_Mist',                container: true,  construct: Jun_Mist         , jun0: true      },
    Jun_ImageParticle:    { name: 'Jun_ImageParticle',                container: false,  construct: Jun_ImageParticle         , jun0: true      },
    Jun_MythicSkill:    { name: 'Jun_MythicSkill',                container: false,  construct: Jun_MythicSkill         , mm: true      },
	Jun_ValuePlus:           { name: 'Jun_ValuePlus',           container: false, construct: Jun_ValuePlus     ,   jun1: true    },
    Jun_SphereParticle:  { name: 'Jun_SphereParticle',            container: false,  construct: Jun_SphereParticle, jun0: true },
    Jun_Mist:  { name: 'Jun_Mist',            container: false,  construct: Jun_Mist, jun0: true },
    Jun_RepeatPlus:  { name: 'Jun_RepeatPlus',            container: false,  construct: Jun_RepeatPlus, jun1: true },
    Jun_TriggerPlus:             { name: 'Jun_TriggerPlus',             container: false,  construct: Jun_TriggerPlus,      jun1: true      },
    Jun_BuffEffect:    { name: 'Jun_BuffEffect',                container: false,  construct: Jun_BuffEffect         , jun0: true      },
    Jun_SnowStorm:    { name: 'Jun_SnowStorm',                container: false,  construct: Jun_SnowStorm         , snowstorm: true      },
    Jun_MM_Mechanic:    { name: 'Jun_MM_Mechanic',                container: false,  construct: Jun_MM_Mechanic         , mm: true      },
    Germ:                { name: 'GermEffect',          container: false,  construct: Germ  , jun2: true     }
...
Component.prototype.createBuilderHTML = function(target)
{
  ...
  //                                                                });
    ...
    remove.style.background = 'url("editor/img/delete.png") no-repeat center rgb(39, 39, 39)';
    ...
    if (this.type != Type.TRIGGER)
    {
        ...
        label.appendChild(duplicate);
    }


    var edit = document.createElement('div');
    edit.title = 'edit';
    edit.innerHTML = 'Edit Settings';
    edit.className = 'builderButton2 smallButton editclass';
    edit.style.width = "24%"
    edit.component = this;
...
function TriggerPhysicalDamage()
{
    ...
    // this.data.push(new ListValue('取消本次伤害', 'canceldamage', [ 'True', 'False' ], 'False')
    // .setTooltip('取消本次伤害 但是触发效果')
// );
// this.data.push(new ListValue('0点伤害取消本次伤害', 'zerocanceldamage', [ 'True', 'False' ], 'False')
// .setTooltip('取消本次伤害 但是触发效果')
// );
...
//     this.data.push(new StringValue("本次伤害记录至value", "ThisDamageValue", "none")
//     .setTooltip('本次伤害记录至value')
// );
}


extend('JUN_CLICK_MONITOR', 'Component');
function JUN_CLICK_MONITOR()
{
    this.super('Jun Click Monitor', Type.TRIGGER, true);

    this.description = '左/右键点击时触发';

    this.data.push(new ListValue('cancelEvent', 'Cancel_original', [ 'true', 'false' ], 'false')
        .setTooltip('取消后,攻击/交互都会失效 并释放技能')
    );
        this.data.push(new ListValue('isRightClick', 'isRightClick', [ 'true', 'false' ], 'false')
        .setTooltip('是否是右键点击时触发,false就是左键')
    );
    this.data.push(new StringListValue('blackItemTypeList', 'blackItemTypeList', '')
.setTooltip('')
);
this.data.push(new StringListValue('whiteItemTypeList', 'whiteItemTypeList', '')
.setTooltip('')
);
this.data.push(new ListValue('debug', 'debug', [ 'True', 'False' ], 'False')
.setTooltip('获取当前手上物品的ID')
);
}


extend('JUN_CAST_MONITOR', 'Component');
function JUN_CAST_MONITOR()
{
    this.super('Jun Cast Monitor', Type.TRIGGER, true);

    this.description = '释放技能时触发';

    this.data.push(new ListValue('all', 'all', [ 'True', 'False' ], 'Flase')
        .setTooltip('全部技能触发')
    );
    this.data.push(new StringListValue('指定技能', 'skillList', [ ] )
        .setTooltip('如果all=true，此处则为排除选项.')
    );
    this.data.push(new StringValue('getSkillValue', 'getskillvalue', 'none')
    .setTooltip('释放的技能名保存到value里')
);
this.data.push(new StringValue('getManaValue', 'getmanavalue', 'none')
.setTooltip('释放的技能所需的法力保存到value里')
);
this.data.push(new StringValue('getSkillName', 'getskillname', 'none')
.setTooltip('释放的技能的技能名保存到value里')
);
this.data.push(new StringValue('getSkilLlevel', 'getskillname', 'none')
.setTooltip('释放的技能的技能等级保存到value里')
);
}



extend('Jun_CastTirrger', 'Component');
function Jun_CastTirrger()
{
    this.super('Jun_CastTirrger', Type.TRIGGER, true);

    this.description = '释放技能时触发此监听器';
...
function TriggerTookPhysicalDamage()
{
    ...
    this.description = 'Applies skill effects when a player takes physical (or non-skill) damage. This includes melee attacks and projectiles not fired by a skill.可在此处将伤害值记录至value,请查阅源代码';
...
//     this.data.push(new ListValue("HORIZONTAL", "HORIZONTAL", ['True', 'False'], 'False')
//     .setTooltip('是否计算指针Y方向')
// );
    this.data.push(new ListValue("Include Caster", "caster", [ 'True', 'False' ], 'False')
        .setTooltip('Whether or not to include the caster in the target list')
    );
...
extend('Jun_Indicator', 'Component');
function Jun_Indicator()
{
    var reflectList = [ 'True'];
    this.super('Jun_Indicator', Type.TARGET, true);

    this.description = '施法流程 第一次触发不释放技能 创建ClickTrigger 创建value key=SpellIndicator,value=skillName+:indicator .触发trigger后 将value设置为value=skillName+:cast 触发技能效果并进入冷却.使用者只需要关注Value的值是:indicator时代表施法 :cast代表释放.可在指示器中内置imgParticle,引擎会自动在触发监听后删除对应特效的key.指示器特效统一Key: SpellIndicator 每次触发/替换指示器的trigger都会删除';
...
this.data.push(new ListValue("pitch-set", "LOOK-H", ['True', 'False'], 'False')
.setTooltip('原版offset不会计算pitch方向 所以开启这里')
);
this.data.push(new ListValue("pitch-addon", "pitcha", ['True', 'False'], 'False')
.setTooltip('在原基础上附加视角偏移量')
);
this.data.push(new AttributeValue('pitch-addon-value', 'pitch', -1, 0).requireValue('pitcha', reflectList)
.setTooltip('在原基础上附加视角偏移量')
);
this.data.push(new ListValue("yaw-addon", "yawa", ['True', 'False'], 'False')
.setTooltip('在原基础上附加视角偏移量')
);

    this.data.push(new AttributeValue('yaw-addon-value', 'yaw', -1, 0).requireValue('yawa', reflectList)
    .setTooltip('在原基础上附加视角偏移量')
    );

    this.data.push(new DoubleValue('☆ maxTime ☆', 'maxTime', 10)
        .setTooltip('秒')
    );
    this.data.push(new ListValue("☆ landBlock ☆", "landblock", ['True', 'False'], 'False')
    .setTooltip('获取地面方块')
);
    this.data.push(new AttributeValue('☆ maxArea ☆', 'pitchForward', 5, 0)
    .setTooltip('casterのpitch决定施法距离,抬头20°为最大距离,低头20°为最小施法距离(0)')
    );
    this.data.push(new AttributeValue('☆ cylinderSensitivity ☆', 'cylinderSensitivity', 0, 0)
    .setTooltip('灵敏度本来是20,在这里可以修改它的附加值,填1就等于灵敏度是21')
    ); 
    this.data.push(new DoubleValue('☆ maxLandBlockDistance ☆', 'maxLandBlockDistance', 5)
.setTooltip('最大吸附地面距离')
);
//         this.data.push(new StringValue('看向Remember', '看向Remember', 'none')
//         .setTooltip('The name of the attribute to add to')
//     );
}


extend('Jun_OffsetPlus', 'Component');
function Jun_OffsetPlus()
{
    var reflectList = [ 'True'];
    this.super('Jun_OffsetPlus', Type.TARGET, true);
...
this.data.push(new ListValue("pitch-set", "LOOK-H", ['True', 'False'], 'True')
.setTooltip('原版offset不会计算pitch方向 所以开启这里')
);
this.data.push(new ListValue("pitch-addon", "pitcha", ['True', 'False'], 'False')
.setTooltip('Whether or not to allow targets to be on the other side of a wall')
);
this.data.push(new AttributeValue('pitch', 'pitch', -1, 0).requireValue('pitcha', reflectList)
.setTooltip('The offset from the target to their right. Negative numbers go to the left.')
);
this.data.push(new ListValue("yaw-addon", "yawa", ['True', 'False'], 'False')
.setTooltip('Whether or not to allow targets to be on the other side of a wall')
    ...
    this.data.push(new AttributeValue('yaw', 'yaw', -1, 0).requireValue('yawa', reflectList)
    .setTooltip('Whether or not to allow targets to be on the other side of a wall')
    );
...
//     .setTooltip('The offset from the target to their right. Negative numbers go to the left.')
//     );
//     this.data.push(new AttributeValue('yaw', 'yaw', -1, 0)
//     .setTooltip('Whether or not to allow targets to be on the other side of a wall')
//     );
//     this.data.push(new ListValue("获取地面方块", "获取地面方块", ['True', 'False'], 'False')
//     .setTooltip('Whether or not to allow targets to be on the other side of a wall')
// );
// this.data.push(new ListValue("附加pitch", "附加pitch", ['True', 'False'], 'False')
// .setTooltip('Whether or not to allow targets to be on the other side of a wall')
// );
// this.data.push(new ListValue("附加yaw", "附加yaw", ['True', 'False'], 'False')
// .setTooltip('Whether or not to allow targets to be on the other side of a wall')
// );
// this.data.push(new ListValue("计算指针Y方向", "计算Y方向", ['True', 'False'], 'False')
// .setTooltip('Whether or not to allow targets to be on the other side of a wall')
// );
//         this.data.push(new StringValue('看向Remember', '看向Remember', 'none')
//         .setTooltip('The name of the attribute to add to')
...
extend('Jun_MM_Target', 'Component');
function Jun_MM_Target()
{
    this.super('Jun_MM_Target', Type.TARGET, true);
    
    this.description = 'Use MythicMobs Select Target';
    
    this.data.push(new StringValue('mechanicName', 'mechanicName', 'Ring')
        .setTooltip('')
    );
    this.data.push(new StringValue('skillCode', 'skillCode', '{radius=5;points=70}')
    .setTooltip('')
);
}

extend('Jun_MythicSkill', 'Component');
function Jun_MythicSkill()
{
    this.super('Jun_MythicSkill', Type.MECHANIC, false);
    
    this.description = 'Cast MythicMobs Skills';
...
extend('Jun_SkyBox', 'Component');
function Jun_SkyBox()
{
    this.super('Jun_SkyBox', Type.MECHANIC, false);
    
    this.description = '天空盒';
    
    this.data.push(new AttributeValue('time', 'time', 10, 0)
        .setTooltip('')
    );
    this.data.push(new StringValue('skyBox1', 'skyBox1', '')
        .setTooltip('静态图片链接')
    );
    this.data.push(new StringValue('skyBox2', 'skyBox2', '')
    .setTooltip('静态图片链接')
);
this.data.push(new StringValue('skyBox3', 'skyBox3', '')
.setTooltip('静态图片链接')
);
this.data.push(new StringValue('skyBox4', 'skyBox4', '')
.setTooltip('静态图片链接')
);
this.data.push(new StringValue('skyBox5', 'skyBox5', '')
.setTooltip('静态图片链接')
);
this.data.push(new StringValue('skyBox6', 'skyBox6', '')
.setTooltip('静态图片链接')
);
}


extend('Jun_Mist', 'Component');
function Jun_Mist()
{
    this.super('Jun_Mist', Type.MECHANIC, false);
    
    this.description = '迷雾';
    
    this.data.push(new AttributeValue('毫秒maxtime', 'maxtime', 10000, 0)
        .setTooltip('毫秒')
    );
    this.data.push(new AttributeValue('power', 'power', 1, 0)
    .setTooltip('')
);
}
...
extend('Jun_ValueString', 'Component');
function Jun_ValueString()
{
    this.super('Jun_ValueString', Type.CONDITION, true);

    this.description = '字符串比较是否相等 支持正则表达式 支持变量%skillName%';

    this.data.push(new StringValue('valueKey', 'valueKey', 'SpellIndicator')
        .setTooltip('The unique string used for the value set by the Value mechanics.')
    );
    this.data.push(new StringValue('valueString', 'valueString', '%skillName%:indicator')
        .setTooltip('支持正则表达式')
    );
}
...
function MechanicDamage()
{
    ...
    this.data.push(new ListValue('关闭击退效果(MM伤害机制)', 'noknock', [ 'True', 'False' ], 'False')
        .setTooltip('Whether or not to deal true damage. True damage ignores armor and all plugin checks.')
    );
    this.data.push(new StringValue('Classifier', 'classifier', 'default')
        .setTooltip('[PREMIUM ONLY] 默认是物理伤害，以做出 魔法伤害')
    );
}
...
function MechanicDamageLore()
{
    ...
    this.data.push(new StringValue('Regex', 'regex', '» 伤害: {value}')
...
function MechanicExplosion()
{
    ...
    this.data.push(new ListValue('Fire', 'fire', [ 'True', 'False' ], 'False')
        ...
        this.data.push(new IntValue('r1', 'r1', 1)
        .setTooltip('The quantity of the item to give to the player')
    );
            this.data.push(new IntValue('g1', 'g1', 1)
        .setTooltip('The quantity of the item to give to the player')
    );
            this.data.push(new IntValue('b1', 'b1', 1)
        .setTooltip('The quantity of the item to give to the player')
    );
            this.data.push(new IntValue('r2', 'r2', 1)
        .setTooltip('The quantity of the item to give to the player')
    );
            this.data.push(new IntValue('g2', 'g2', 1)
        .setTooltip('The quantity of the item to give to the player')
    );
            this.data.push(new IntValue('b2', 'b2', 1)
        .setTooltip('The quantity of the item to give to the player')
    );
        this.data.push(new ListValue("类型", "类型", [ "BALL", "BALL_LARGE","STAR","BURST","CREEPER" ], "BURST")
...
extend('Jun_SnowStorm', 'Component');
function Jun_SnowStorm()
{
    this.super('Jun_SnowStorm', Type.MECHANIC, false);

    this.description = '释放暴风雪引擎特效 不会占用大量带宽并且自由度更高 预计在29版本加入';

//     this.data.push(new ListValue('inEntity', 'inEntity', [ 'True', 'False' ], 'False')
//     .setTooltip('跟随')
// );
// this.data.push(new StringListValue('varableList', 'varableList', ["key:value"])
// .setTooltip('自定义变量')
// );
this.data.push(new StringValue('effectName', 'effectName', '特效名称')
.setTooltip('')
);
this.data.push(new AttributeValue('pitch-addon', 'pitch-addon', 0, 0)
.setTooltip('')
);
this.data.push(new AttributeValue('yaw-addon', 'yaw-addon', 0, 0)
.setTooltip('')
);
...
extend('Jun_ValuePlus', 'Component');
function Jun_ValuePlus()
{
    this.super('Jun_ValuePlus', Type.MECHANIC, false);
    
    this.description = 'formula允许的变量: {value} %PAPI% #skillLevel# #targetSize#(选定目标数)';
    ...
    this.data.push(new StringValue('Key', 'key', 'value')
	    ...
	    this.data.push(new StringValue('formula', 'formula', '1+{value}')
        .setTooltip('解析value,papi,并进行数学运算 除加减乘除外还可以支持超复杂的逻辑运算: & | ^ ~ << >> >>>> ++ -- ?: += -= *= /= %= <= |= ^= <= >>= >>>=......round(x,1) round(x) left(str,len) ceil(x) floor(x) rint(x) avg&min&max&vsum(x1,x2,x3..) sin(x) cos(x) tan(x) asin(x) acos(x) atan(x) atan2(y,x) sec(x) cosec(x) cot(x) sinh(x) tanh(x) asinh(x) acosh(x) atanh(x) PI e ...还有很多公式没写出来 不懂随时问我')
    );
    this.data.push(new StringValue('valueString', 'valueString', '{caster}的计算结果@formula@')
        .setTooltip('@formula@ 为计算好的数 变量都可以使用: %papi% {caster} {target} {value}')
    );
    this.data.push(new AttributeValue('clearTime', 'clearTime', -1, 0)
    .setTooltip('移除时间(秒) 小于等于0 为不移除')
);
	    this.data.push(new ListValue('deathRemove', 'deathRemove', [ 'True', 'False' ], 'True')
        .setTooltip('死亡是否移除,服务器重启后仍会移除')
    );
		    this.data.push(new ListValue('text', 'text', [ 'True', 'False' ], 'False')
        .setTooltip('向施法者发送valueString')
    );
}
...
// 	this.data.push(new AttributeValue('逆精准', '逆精准', 0, 0)
//         .setTooltip('随机偏')
//     );
//     this.data.push(new ListValue('反重力', '反重力', ['True', 'False'], 'False')
// .setTooltip('反重力')
// );
	// this.data.push(new AttributeValue('持续时间', '持续时间', -1, 0)
    //     .setTooltip('持续时间')
    // );
...
extend('LookTargetMechanic', 'Component');
function LookTargetMechanic()
{
    this.super('LookTargetMechanic', Type.MECHANIC, false);

    this.description = 'Launches the target relative to their forward direction. Use negative values to go in the opposite direction (e.g. negative forward makes the target go backwards)';

    this.data.push(new StringValue('getRemember', 'getRemember', 'none')
    .setTooltip('Remember对目标施法')
);
...
Jun_MM_Mechanic
extend('Jun_MM_Mechanic', 'Component');
function Jun_MM_Mechanic()
{
    this.super('Jun_MM_Mechanic', Type.MECHANIC, false);

    this.description = '释放MythicMobs一段效果代码';

    this.data.push(new StringValue('mechanicName', 'mechanicName', 'JUMP')
        .setTooltip('效果名')
    );
    this.data.push(new StringValue('skillCode', 'skillCode', '{v=20}')
    .setTooltip('效果代码')
);

}

extend('Jun_BuffEffect', 'Component');
function Jun_BuffEffect()
{
    this.super('Jun_BuffEffect', Type.MECHANIC, false);

    this.description = '播放一个获取buff的特效';



    this.data.push(new DoubleValue('[TICK]time', 'particleMaxAge',100)
        .setTooltip('最大时间 - tick.')
    );
    this.data.push(new DoubleValue('particleRed', 'particleRed', 255)
        .setTooltip('How far above the target in blocks to play the particles. A negative value will go below.')
    );
    this.data.push(new DoubleValue('particleGreen', 'particleGreen', 255)
        .setTooltip('How far to the right of the target to play the particles. A negative value will go to the left.')
    );
    this.data.push(new DoubleValue('particleBlue', 'particleBlue', 255)
    .setTooltip('How far to the right of the target to play the particles. A negative value will go to the left.')
);
this.data.push(new DoubleValue('【数量】hrepeat', 'hrepeat', 1)
.setTooltip('How far to the right of the target to play the particles. A negative value will go to the left.')
);
this.data.push(new DoubleValue('【尺寸】size', 'size', 1)
.setTooltip('How far to the right of the target to play the particles. A negative value will go to the left.')
);
this.data.push(new DoubleValue('【不透明度0~1】particleAlpha', 'particleAlpha', 1)
.setTooltip('How far to the right of the target to play the particles. A negative value will go to the left.')
);
this.data.push(new DoubleValue('【y方向移动】yaddspeed', 'yaddspeed', 0)
.setTooltip('How far to the right of the target to play the particles. A negative value will go to the left.')
);
this.data.push(new ListValue('【绑定实体】inentity', 'inentity', [ 'True', 'False' ], 'False')
.setTooltip('跟随实体,不推荐在非实体上使用(offset也是非实体),客户端可能检测不到')
);
}
...
extend('Jun_ParticleAnimation', 'Component');
function Jun_ParticleAnimation()
{
    this.super('Jun_ParticleAnimation', Type.MECHANIC, false);
    ...
    this.data.push(new IntValue('Start Angle', 'start', 0)
        .setTooltip('The starting orientation of the animation. Horizontal translations and the forward/right offsets will be based off of this.')
		    ...
		    this.data.push(new IntValue('🌒AngleXZ', 'angleXZ', 0)
        .setTooltip('YAW轴旋转')
    );
	    this.data.push(new IntValue('🌒Start AngleXZ', 'startXZ', 0)
        .setTooltip('The starting orientation of the animation. Horizontal translations and the forward/right offsets will be based off of this.')
    );
    this.data.push(new AttributeValue('Duration', 'duration', 5, 0)
        .setTooltip('How long the animation should last for in seconds')
    );
    this.data.push(new AttributeValue('H-Translation(平面移动)', 'h-translation', 0, 0)
    ...
    this.data.push(new AttributeValue('V-Translation(垂直移动)', 'v-translation', 0, 0)
...
function MechanicParticleAnimation()
{
    ...
    this.data.push(new AttributeValue('H-Translation平面移动)', 'h-translation', 0, 0)
        .setTooltip('How far the animation moves horizontally relative to the center over a cycle. Positive values make it expand from the center while negative values make it contract.')
    );
    this.data.push(new AttributeValue('V-Translation(垂直移动)', 'v-translation', 0, 0)
        .setTooltip('How far the animation moves vertically over a cycle. Positive values make it rise while negative values make it sink.')
...
extend('Jun_ImageParticle', 'Component');
function Jun_ImageParticle()
{
    this.super('Jun_ImageParticle', Type.MECHANIC, false);
    
    this.description = '释放图片特效';
    
    this.data.push(new AttributeValue('持续时间-life(秒)', 'life', 1, 0)
    .setTooltip('')
    );

this.data.push(new AttributeValue('重播次数-repeat', 'repeat', 0, 0)
.setTooltip('图片重播,0则无限重播')
);
this.data.push(new AttributeValue('帧播放速度-readSpeed', 'readspeed', 15, 0)
.setTooltip('每帧自增,增加至60切换')
);
this.data.push(new ListValue('绑定实体-bindEntity', 'inentity', [ 'True', 'False' ], 'False')
.setTooltip('跟随实体,不推荐在非实体上使用(offset也是非实体),客户端可能检测不到')
);
var reflectList = [ 'True'];
var reflectList2 = [ 'Cylinder'];
var reflectList3 = [ 'Text'];

this.data.push(new ListValue('[bindEntity]实时更新方向', 'onentityupdate', [ 'True', 'False' ], 'False').requireValue('inentity', reflectList)
.setTooltip('开启后可以让偏移,自转都随实体视角而改变')
);




this.data.push(new ListValue('图片视角锁定-inLook', 'inlook', [ 'True', 'False' ], 'True')
.setTooltip('固定视角')
);
this.data.push(new ListValue('不锁定上下视角', 'LAH', [ 'True', 'False' ], 'false').requireValue('inlook', reflectList)
.setTooltip('固定视角')
);

this.data.push(new ListValue('isGif', 'isgif', [ 'True', 'False' ], 'False')
.setTooltip('GIF模式.只能传一个链接')
);
this.data.push(new StringListValue('图片链接-imgLinks', 'links', [])
.setTooltip('传多个连接进来自动拼成GIF，禁止用中文')
);


this.data.push(new ListValue('🍀 基础效果 Base 🍀', '编辑器参数-Base-', [ 'True', 'False' ], 'True')
.setTooltip('')
);
this.data.push(new AttributeValue('🍀 初始宽度 WIDTH-startSize', 'startWidthSize', 1, 0)
.setTooltip('初始尺寸')
);
this.data.push(new AttributeValue('🍀 初始长度 HIGHT-startSize', 'startHightSize', 1, 0)
.setTooltip('初始尺寸')
);
this.data.push(new AttributeValue('🍀 初始YAW YAW-startAngle', 'startYawAngle', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('初始角度 YAW轴角度')
);
this.data.push(new AttributeValue('🍀 初始PITCH startAngle', 'startPitchAngle', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('初始角度 Ptich轴角度')
);
this.data.push(new AttributeValue('🍀 初始ROW startAngle', 'startRowAngle', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('初始角度 ROW轴角度')
);
this.data.push(new AttributeValue('🍀 PITCH递增 uniformAngle', 'UVpitch', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('匀速角度旋转')
);
this.data.push(new AttributeValue('🍀 YAW递增 YAW-uniformAngle', 'UVyaw', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('匀速角度旋转')
);
this.data.push(new AttributeValue('🍀 ROW递增 uniformAngle', 'UVrow', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('匀速角度旋转')
);
this.data.push(new AttributeValue('🍀 前偏移-forwardOffset', 'forwardoffset', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('随时间附加 前偏移')
);
this.data.push(new AttributeValue('🍀 上偏移-upwardOffset', 'upoffset', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('随时间附加 上偏移')
);
this.data.push(new AttributeValue('🍀 右偏移-rightwardOffset', 'rightoffset', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('随时间附加 右偏移')
);
this.data.push(new AttributeValue('🍀 向前速度-forWardSpeed', 'forwardspeed', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('随时间向前的速度')
);

this.data.push(new AttributeValue('🍀 向上速度-upSpeed', 'upspeed', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('随时间向上的速度')
);
this.data.push(new AttributeValue('🍀 向右速度-rightSpeed', 'rightspeed', 0, 0).requireValue('编辑器参数-Base-', reflectList)
.setTooltip('随时间向右的速度')
);
this.data.push(new ListValue('⚙ 变速效果 ZOOM ⚙', '编辑器参数-biansu-', [ 'True', 'False' ], 'False')
.setTooltip('变速缩放动画持续时间=Life / ZoomTime')
);



this.data.push(new AttributeValue(' ⚙ WIDTH ZOOM', 'width', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('图片宽度')
);
this.data.push(new AttributeValue(' ⚙ WIDTH TIME', 'zoomWidthTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('此值越小变速用时越多,越多用时越少')
);
this.data.push(new AttributeValue(' ⚙ HIGHT ZOOM', 'hight', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('图片长度')
);
this.data.push(new AttributeValue(' ⚙ HIGHT TIME', 'zoomHightTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('此值越小变速用时越多,越多用时越少')
);
this.data.push(new AttributeValue(' ⚙ PITCH ZOOM', 'pitchangle', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('随时间附加角度 与ZOOM TIME配合使用')
);
this.data.push(new AttributeValue(' ⚙ PITCH TIME', 'zoomPitchAngleTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('此值越小变速用时越多,越多用时越少')
);
this.data.push(new AttributeValue(' ⚙ YAW ZOOM', 'yawangle', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('随时间附加角度 与ZOOM TIME配合使用')
);
this.data.push(new AttributeValue(' ⚙ YAW TIME', 'zoomYawAngleTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('此值越小变速用时越多,越多用时越少')
);
this.data.push(new AttributeValue(' ⚙ ROW ZOOM', 'zoomrowangle', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('随时间附加角度 与ZOOM TIME配合使用')
);
this.data.push(new AttributeValue(' ⚙ ROW TIME', 'zoomRowAngleTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('此值越小变速用时越多,越多用时越少')
);
this.data.push(new AttributeValue(' ⚙ FORWARD ZOOM', 'forwardspeedzoom', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('')
);
this.data.push(new AttributeValue(' ⚙ FORWARD TIME', 'zoomforwardTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('')
);

this.data.push(new AttributeValue(' ⚙ RIGHT ZOOM', 'rightspeedzoom', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('')
);

this.data.push(new AttributeValue(' ⚙ RIGHT TIME', 'zoomrightTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('')
);
this.data.push(new AttributeValue(' ⚙ UP ZOOM', 'upspeedzoom', 0, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('')
);
this.data.push(new AttributeValue(' ⚙ UP TIME', 'zoomupdateTime', 2, 0).requireValue('编辑器参数-biansu-', reflectList)
.setTooltip('')
);


this.data.push(new ListValue('🕶️ 视角锁定 Angle 🕶️', '编辑器参数-shijiao-', [ 'True', 'False' ], 'False')
.setTooltip('')
);
this.data.push(new ListValue('🕶️ 偏移角度平行地面Forward', 'HORIZONTAL', [ 'True', 'False' ], 'False').requireValue('编辑器参数-shijiao-', reflectList)
.setTooltip('Forward角度计算时候 与地面平行')
);
this.data.push(new ListValue('🕶️ 移动角度平行地面ForwardSpeed', 'forwardmove', [ 'True', 'False' ], 'False').requireValue('编辑器参数-shijiao-', reflectList)
.setTooltip('ForwardSpeed角度计算时 与地面平行')
);
this.data.push(new ListValue('🕶️ 自转角度平行地面', 'picthlock', [ 'True', 'False' ], 'False').requireValue('编辑器参数-shijiao-', reflectList)
.setTooltip('忽略Pitch，主要用于[自转角度]的计算，但是会计算技能里其它参数设置的')
);
this.data.push(new ListValue('🕶️ 肢体绑定', 'body', [ 'body', 'eye' ], 'eye').requireValue('编辑器参数-shijiao-', reflectList)
.setTooltip('使用眼的yaw还是身体的yaw')
);
// this.data.push(new ListValue('Relativeper_spective', 'Relativeperspective', [ 'absolute_angle', '' ], '').requireValue('编辑器参数-shijiao-', reflectList)
// .setTooltip('图片在生成时不再以面向施法者的方式计算角度absolute_angle')
// );
this.data.push(new ListValue('🕶️ 强制附加角度', 'Relativeperspective', [ '', 'inlookaddangle' ], '').requireValue('编辑器参数-shijiao-', reflectList)
.setTooltip('lookaddangle-在inlook时仍计算设置的角度')
);

// this.data.push(new ListValue('平行地面-horizontal-Pitch', 'HORIZONTAL', [ 'True', 'False' ], 'True').requireValue('编辑器参数-shijiao-', reflectList)
// .setTooltip('玩家的Pitch')
// );
this.data.push(new ListValue('🕶️ 忽略视角-Yaw', 'HORIZONTAL-YAW', [ 'True', 'False' ], 'False').requireValue('编辑器参数-shijiao-', reflectList)
.setTooltip('一般不用')
);

this.data.push(new ListValue('🌏 公转 Revolution🌏', '编辑器参数-gongzhuan-', [ 'True', 'False' ], 'False')
.setTooltip('')
);
this.data.push(new AttributeValue('🌏 平行初始角度 startRevolutionH', 'rotateSH', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('Start angle of revolution, HORIZONTAL')
);
this.data.push(new AttributeValue('🌏 垂直初始角度 startRevolutionV', 'rotateSV', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('Start angle of revolution, vertical')
);
this.data.push(new AttributeValue('🌏 平行旋转角度 RevolutionH', 'rotateH_animation', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('revolution HORIZONTAL angle')
);
this.data.push(new AttributeValue('🌏 垂直旋转角度 RevolutionV', 'rotateV_animation', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('revolution vertical angle')
);
this.data.push(new AttributeValue('🌏 平行半径初始值 startRevolutionH-A', 'rotateHAreaStart', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('revolution HORIZONTAL angle, HORIZONTAL AREA')
);
this.data.push(new AttributeValue('🌏 垂直半径初始值 startRevolutionV-A', 'rotateVAreaStart', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('revolution vertical angle, vertical AREA')
);
this.data.push(new AttributeValue('🌏 平行半径递增 RevolutionH-A-Zoom', 'rotateHAreaSpeed', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('revolution HORIZONTAL angle, HORIZONTAL AREA zoom')
);
this.data.push(new AttributeValue('🌏 垂直半径递增 RevolutionV-A-Zoom', 'rotateVAreaSpeed', 0, 0).requireValue('编辑器参数-gongzhuan-', reflectList)
.setTooltip('revolution vertical angle, vertical AREA zoom')
);
this.data.push(new ListValue('❀ 颜色效果 Color ❀', '编辑器参数-yanse-', [ 'True', 'False' ], 'False')
.setTooltip('')
);
this.data.push(new AttributeValue('❀ ColorR', 'ColorR', 255, 0).requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('revolution vertical angle, vertical AREA zoom')
);
this.data.push(new AttributeValue('❀ ColorG', 'ColorG', 255, 0).requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('revolution vertical angle, vertical AREA zoom')
);
this.data.push(new AttributeValue('❀ ColorB', 'ColorB', 255, 0).requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('revolution vertical angle, vertical AREA zoom')
);
this.data.push(new AttributeValue('❀ RGBAstart透明度初始值', 'RGBAstart', 0, 0).requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('透明度为0(不透明)-1(透明)')
);
this.data.push(new ListValue('❀ 图片绑定类型-IMGbindtype', 'bindtype', [ 'GL_TEXTURE_1D', 'GL_TEXTURE_2D','GL_TEXTURE_3D','' ], '').requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('建议留空')
);
// this.data.push(new ListValue('淡出', 'colortype', [ 'fade_out', 'normal' ], 'normal').requireValue('编辑器参数-yanse-', reflectList)
// .setTooltip('测试参数')
// );
this.data.push(new ListValue('❀ 混色-源因子', 'blendFunc1', [ 'GL_ONE', 'GL_ZERO','GL_SRC_COLOR','GL_DST_COLOR','GL_ONE_MINUS_SRC_COLOR'
,'GL_ONE_MINUS_DST_COLOR','GL_SRC_ALPHA','GL_DST_ALPHA','GL_ONE_MINUS_SRC_ALPHA','GL_ONE_MINUS_DST_ALPHA' ,'GL_SRC_ALPHA_SATURATE','Profile:DEFAULT','Profile:PLAYER_SKIN','Profile:TRANSPARENT_MODEL',"disable",''
,"GL_POLYGON_OFFSET_FACTOR",
"GL_POLYGON_OFFSET_POINT","GL_POLYGON_OFFSET_LINE","GL_POLYGON_OFFSET_FILL","GL_POLYGON_OFFSET_UNITS","tryBlendFuncSeparate" ], '').requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('源于OpenGL中的blendFunc,两个都为空则使用默认混色,disable为彻底关闭混色')
);
this.data.push(new ListValue('❀ 混色-目标因子', 'blendFunc2', [ 'GL_ONE', 'GL_ZERO','GL_SRC_COLOR','GL_DST_COLOR','GL_ONE_MINUS_SRC_COLOR'
,'GL_ONE_MINUS_DST_COLOR','GL_SRC_ALPHA','GL_DST_ALPHA','GL_ONE_MINUS_SRC_ALPHA','GL_ONE_MINUS_DST_ALPHA' ,'GL_SRC_ALPHA_SATURATE',"disable","GL_POLYGON_OFFSET_FACTOR" ,''], '').requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('源于OpenGL中的blendFunc,两个都为空则使用默认混色,disable为彻底关闭混色')
);

this.data.push(new AttributeValue('❀ 透明度递增 RGBAuniform', 'RGBadimation', 0, 0).requireValue('编辑器参数-yanse-', reflectList)
.setTooltip('透明度为0(不透明)-1(透明)')
);
this.data.push(new ListValue('✡ 其它 Other ✡', '编辑器参数-OTHER-', [ 'True', 'False' ], 'False')
.setTooltip('')
);
this.data.push(new ListValue('仅施法者可见-onlyCasterLook', 'only_Caster_Can_Look', [ 'True', 'False' ], 'False').requireValue('编辑器参数-OTHER-', reflectList)
.setTooltip('')
);
this.data.push(new StringValue('✡ ParticleKeyPutInValue', 'particlekey', '%playerUUID%SpellIndicator').requireValue('编辑器参数-OTHER-', reflectList)
.setTooltip('将此粒子的ID值加入到指定value中，主要用于删除粒子. 变量:%playerUUID%')
);

this.data.push(new ListValue('✡ 绘制类型-drawType', 'drawType', [ 'Normal', 'Orthogonal','Scale','Sphere','Cylinder','Cube','Text' ,"WALL"], 'normal').requireValue('编辑器参数-OTHER-', reflectList)
.setTooltip('Orthogonal-交叉绘制 Box-长方体 Cube-正方体(尺寸=width) Scale-带有厚度的图片 Sphere-3d球(尺寸=width) Cylinder-圆柱(底部半径=宽度,高度=长度) WALL-墙壁(四个面)')
);
this.data.push(new StringValue('drawText', 'drawText', 'text').requireValue('drawType', reflectList3)
.setTooltip('绘制的图片')
);
this.data.push(new AttributeValue('✡ 顶部初始尺寸 CylinderTopStartSize', 'CylinderTopSize', 0, 0).requireValue('drawType', reflectList2)
.setTooltip('圆柱底部尺寸为width，顶部尺寸为CylinderTopSize')
);
this.data.push(new AttributeValue('✡ 顶部尺寸递增 CylinderTop', 'CylinderTop', 0, 0).requireValue('drawType', reflectList2)
.setTooltip('圆柱底部尺寸为width，顶部尺寸为CylinderTopSize')
);
this.data.push(new ListValue('☆ indicator ☆', 'indicator', [ 'True', 'False' ], 'False')
.setTooltip('指示器效果')
);
this.data.push(new ListValue('☆ land ☆', 'land', [ 'True', 'False' ], 'True').requireValue('indicator', reflectList)
.setTooltip('特效吸附于地面')
);
this.data.push(new AttributeValue('☆ MaxlandDistance ☆', 'landDistance', 1, 0).requireValue('indicator', reflectList)
.setTooltip('吸附最大距离 上下皆可吸')
);
this.data.push(new AttributeValue('☆ cylinderSensitivity ☆', 'cylinderSensitivity', 0, 0)
.setTooltip('灵敏度本来是20,在这里可以修改它的附加值,填1就等于灵敏度是21')
); 
this.data.push(new AttributeValue('☆ pitchForward ☆', 'maxArea', 5, 0).requireValue('indicator', reflectList)
.setTooltip('指最大距离 pitch决定施法距离,抬头20°为最大距离,低头20°为最小施法距离(0)')
);
...
extend('Jun_ProjectLie', 'Component');
function Jun_ProjectLie()
{
    this.super('Jun_ProjectLie', Type.MECHANIC, true);

    this.description = 'Launches a projectile that applies child components on hit. The target supplied will be the struck target.';

    this.data.push(new ListValue('Projectile', 'projectile', [ 'DragonFireball', 'Egg', 'EnderPearl', 'Fireworks', 'FishingHook', 'LargeFireball', 'LlamaSpit', 'Potion', 'ShulkerBullet'
, 'SmallFireball', 'SnowBall', 'ThrownExpBottle', 'ThrownTrident', 'TippedArrow', 'WitherSkull'	], 'TippedArrow')
        .setTooltip('The type of projectile to fire')
    );
    this.data.push(new ListValue('Flaming', 'flaming', [ 'True', 'False' ], 'False')
        .setTooltip('Whether or not to make the launched projectiles on fire.')
    ...
    this.data.push(new AttributeValue('deviationAngle', 'deviationAngle',0,0)
    .setTooltip('角度随机倍率，为0则不会偏移原来的轨道，0.1会随机')
    );
	    this.data.push(new AttributeValue('xzAngle', 'xzAngle',0,0)
    .setTooltip('竖向角度扩散')
    );
	this.data.push(new ListValue('gravity', 'gravity', [ 'True', 'False' ], 'False')
    .setTooltip('重力')
    );
	this.data.push(new ListValue('break_blocks', 'break_blocks', [ 'True', 'False' ], 'False')
    .setTooltip('爆炸物 破坏方块')
    );

	this.data.push(new StringValue('pickupStatus', 'pickupStatus', 'pickup')
    .setTooltip('被拾取状态.')
    );
		this.data.push(new StringValue('color', 'color', '')
    .setTooltip('药水类抛射物可以拥有颜色 格式可以是#开头或者纯数字.')
    );
	
    this.data.push(new StringListValue('imgList', 'imgList', [])
    .setTooltip('传多个连接进来自动拼成GIF，禁止用中文')
    );
	this.data.push(new ListValue('isGif', 'isGif', [ 'True', 'False' ], 'False')
    .setTooltip('')
    );
    this.data.push(new ListValue('drawType', 'drawType', [ 'normal', 'orthogonal','sphere' ], 'normal')
    .setTooltip('')
    );
	this.data.push(new ListValue('useTrail', 'useTrail', [ 'True', 'False' ], 'True')
    .setTooltip('')
    );
    this.data.push(new AttributeValue('TrailLen', 'TrailLen',10,0)
    .setTooltip('轨迹长度')
    );
    this.data.push(new AttributeValue('TrailSize', 'TrailLen',0.5,0)
    .setTooltip('轨迹大小')
    );
    this.data.push(new DoubleValue('trailR', 'trailR', 1)
    .setTooltip('轨迹颜色RGB/255')
    );
    this.data.push(new DoubleValue('trailG', 'trailG', 1)
    .setTooltip('轨迹颜色RGB/255')
    );
    this.data.push(new DoubleValue('trailB', 'trailB', 1)
    .setTooltip('轨迹颜色RGB/255')
    );
    this.data.push(new DoubleValue('trailA', 'trailA', 1)
    .setTooltip('0~1')
    );
    this.data.push(new StringValue('particleName', 'particleName', '')
    .setTooltip('药水类抛射物可以拥有颜色 格式可以是#开头或者纯数字.')
    );
    this.data.push(new AttributeValue('particleArea', 'particleArea',1,0)
    .setTooltip('')
    );
    this.data.push(new AttributeValue('particleAmount', 'particleAmount',1,0)
    .setTooltip('')
    );
    this.data.push(new AttributeValue('pierceEntityNum', 'pierceEntityNum',1,0)
    .setTooltip('')
    );
    this.data.push(new AttributeValue('pierceBlockNum', 'pierceBlockNum',1,0)
    .setTooltip('')
    );
    this.data.push(new AttributeValue('textureSize', 'textureSize',1,0)
    .setTooltip('')
    );
    this.data.push(new AttributeValue('particleSpeed', 'particleSpeed',1,0)
    .setTooltip('')
    );
    this.data.push(new DoubleValue('sphereR', 'sphereR', 1)
    .setTooltip('0~1')
    );
    this.data.push(new DoubleValue('sphereG', 'sphereG', 1)
    .setTooltip('0~1')
    );
    this.data.push(new DoubleValue('sphereB', 'sphereB', 1)
    .setTooltip('0~1')
    );
    this.data.push(new DoubleValue('sphereA', 'sphereA', 1)
    .setTooltip('0~1')
    );
    addProjectileOptions(this);
    addEffectOptions(this, true);
...
extend('Jun_RepeatPlus', 'Component');
function Jun_RepeatPlus()
{
    this.super('Jun_RepeatPlus', Type.MECHANIC, true);

    this.description = '声明一个value,每次重复都为此value增加一次值,间隔时间可以小于1tick';
    this.data.push(new StringValue('valueKey', 'valueKey', "")
    .setTooltip('valueKey')
);
    this.data.push(new AttributeValue('valueSet', 'valueSet', 0, 0)
        .setTooltip('value初始值')
    );
    this.data.push(new AttributeValue('valueAdd', 'valueAdd', 1, 0)
    .setTooltip('每次循环为value自增的值')
);
    this.data.push(new AttributeValue('Repetitions重复次数', 'repetitions', 3, 0)
        .setTooltip('How many times to activate child components')
    );
    this.data.push(new DoubleValue('Period重复间隔', 'period', 1)
        .setTooltip('重复间隔 秒')
    );
...
//     this.data.push(new ListValue("递归", "递归", ['True', 'False'], 'False')
//     .setTooltip('')
// );
// this.data.push(new ListValue("递归顺序", "递归顺序", ['随机', '最大生命','目前生命'], '随机')
// .setTooltip('')
// );
// this.data.push(new ListValue("递归完成时关闭重复", "递归完成时关闭重复", ['True', 'False'], 'False')
// .setTooltip('')
// );
}
...
function MechanicSpeed()
{
    ...
    this.data.push(new ListValue('Stackable', 'stackable', [ 'True', 'False' ], 'True')
        .setTooltip('设定为True时，将赋予玩家速度buff以计算总速度——所有的FlatNumber将相加得到一个乘数；所有的Multiplier将单独作为乘数。每个技能的单个Modifier下只有（最新的）一个Buff有效。设定为False时，将清空所有速度buff，刷新速度并与(Flat+1)/Multiplier相乘。')
    );
	
    this.data.push(new ListValue('Modifier', 'modifier', [ 'Flat', 'Multiplier'], 'Multiplier')
        .setTooltip('FlatNumber仍为乘数，详见上一项说明')
    );
	
    this.data.push(new AttributeValue('Value', 'value', 1.2, 0)
...
extend('Jun_TriggerPlus', 'Component');
function Jun_TriggerPlus()
{
    this.super('Jun_TriggerPlus', Type.MECHANIC, true);

    this.description = 'Listens for a trigger on the current targets for a duration.once模式下可根据权重排序释放(每次只释放权重最高的),可根据相同权重强行替换,每次释放可以指定value自增';

    this.data.push(new ListValue('Trigger', 'trigger', [ 'Crouch', 'Death', 'Environment Damage', 'Kill', 'Land', 'Launch', 'Physical Damage', 'Skill Damage', 'Took Physical Damage', 'Took Skill Damage' , 'Move','Jun Cast Monitor','Jun Click Monitor'], 'Death')
    ...
    this.data.push(new IntValue('weight', 'weight', 0)

        .setTooltip('权重')
    );

    this.data.push(new ListValue('replaceOneWeight', 'replaceOneWeight', [ 'True', 'False', ], 'True')
    .setTooltip('如果已经存在相同权重的监听器则替换')
);
    this.data.push(new StringValue('valueKey', 'valueKey', "")
    .setTooltip('每次触发此监听 此value+1')
    ...
    this.data.push(new ListValue('Target Listen Target', 'target', [ 'True', 'False' ], 'True')
        ...
        this.data.push(new ListValue('Target Listen Target', 'target', [ 'True', 'False' ], 'True')
        .requireValue('trigger', 'Casttrigger')
        .setTooltip('True makes children target the target that has been listened to. False makes children target the entity fighting the target entity.')
    );
    ...
    var CASTMONITOR = [ 'Jun Cast Monitor'];
    var CLICKMONTIOR=['Jun Click Monitor']
    this.data.push(new ListValue('all', 'all', [ 'True', 'False' ], 'Flase').requireValue('trigger', CASTMONITOR)
        .setTooltip('全部技能触发')
    );
    this.data.push(new StringListValue('指定技能', 'skillList', [ ] ).requireValue('trigger', CASTMONITOR)
        .setTooltip('如果all=true，此处则为排除选项.')
    );
    this.data.push(new StringValue('getSkillValue', 'getskillvalue', 'none').requireValue('trigger', CASTMONITOR)
    .setTooltip('释放的技能名保存到value里')
);
this.data.push(new StringValue('getManaValue', 'getmanavalue', 'none').requireValue('trigger', CASTMONITOR)
.setTooltip('释放的技能所需的法力保存到value里')
);
this.data.push(new StringValue('getSkillName', 'getskillname', 'none').requireValue('trigger', CASTMONITOR)
.setTooltip('释放的技能的技能名保存到value里')
);
this.data.push(new StringValue('getSkilLlevel', 'getskillname', 'none').requireValue('trigger', CASTMONITOR)
.setTooltip('释放的技能的技能等级保存到value里')
);
this.data.push(new ListValue('取消本次点击事件-cancelEvent', 'Cancel_original', [ 'true', 'false' ], 'false').requireValue('trigger', CLICKMONTIOR)
.setTooltip('取消后,攻击/交互都会失效 并释放技能')
);
this.data.push(new ListValue('isRightClick', 'isRightClick', [ 'true', 'false' ], 'false').requireValue('trigger', CLICKMONTIOR)
.setTooltip('是否是右键点击时触发,false就是左键')
);
    //     this.data.push(new ListValue('取消原本触发器效果', '取消原本触发器效果', [ 'True', 'False' ], 'Flase')
    //     .requireValue('trigger','CLICKLISTER')
    //     .setTooltip('True makes children target the caster. False makes children target the damaged entity')
    // );
    //     this.data.push(new ListValue('isRightClick', 'isRightClick', [ 'True', 'False' ], 'Flase')
    //             .requireValue('trigger','CLICKLISTER')
    //     .setTooltip('True makes children target the caster. False makes children target the damaged entity')
    // );
...
function MechanicValueMultiply()
{
    ...
    this.data.push(new StringValue('Key', 'key', '伤害值')
        .setTooltip('The unique key to store the value under. This key can be used in place of attribute values to use the stored value.')
    );
    this.data.push(new AttributeValue('Multiplier', 'multiplier', 1, 0)
...
function MechanicValuePlaceholder()
{
    ...
    this.data.push(new StringValue('Placeholder', 'placeholder', '%ns_damage_cal%')
...
function MechanicValueRandom()
{
    ...
    this.data.push(new StringValue('Key', 'key', 'value')
        .setTooltip('key前面加DIV 则改为除法 The unique key to store the value under. This key can be used in place of attribute values to use the stored value.')
    );
...
extend('Jun_Quake', 'Component');
function Jun_Quake()
{
    this.super('Jun_Quake', Type.MECHANIC, true);
    
    this.description = '两种晃动玩家摄像机的效果';
    


    this.data.push(new ListValue('type', 'type', [ 'normal', 'Explosion_vibration' ], 'normal')
        .setTooltip('normal像地震,Explosion_vibration像手雷爆炸。power的数值会逐渐减少，就算时间没到它也有减没的可能。如果时间到了会强行清空power')
    );
    this.data.push(new AttributeValue('power', 'power', 100, 0)
        .setTooltip('..')
    );
    this.data.push(new AttributeValue('maxtime', 'maxtime', 10000, 0)
    .setTooltip('..')
);
var reflectList = [ 'Explosion_vibration'];

this.data.push(new AttributeValue('[Explosion]maxup', 'maxup1', 100, 0).requireValue('type', reflectList)
.setTooltip('..')
);
this.data.push(new AttributeValue('[Explosion]minup', 'minup', 10, 0).requireValue('type', reflectList)
.setTooltip('..')
);
this.data.push(new AttributeValue('[Explosion]uppower', 'uppower', 100, 0).requireValue('type', reflectList)
.setTooltip('..')
);
this.data.push(new AttributeValue('[Explosion]downpower', 'downpower', 100, 0).requireValue('type', reflectList)
.setTooltip('..')
);
this.data.push(new AttributeValue('[Explosion]backspeed', 'backspeed', 10, 0).requireValue('type', reflectList)
.setTooltip('..')
);

}
extend('Jun_SphereParticle', 'Component');
function Jun_SphereParticle()
{
    this.super('Jun_SphereParticle', Type.MECHANIC, true);
    
    this.description = '生成一个球形,如果想让它只是个普通的球 要先看一下SphereHeight,使用颜色需要speed>0(speed为颜色的亮度)';
    

    this.data.push(new AttributeValue('area-范围', 'area', 1,0)
        .setTooltip('none')
    );

 //   this.data.push(new StringValue('xzangle', 'xzangle', "360")
 //       .setTooltip('none')
 //   );
 //       this.data.push(new StringValue('xyzangle', 'xyzangle', "360")
 //       .setTooltip('none')
 //   );
       this.data.push(new AttributeValue('SphereHeight', 'xyzangleStart', 1,0)
       .setTooltip('球形高度设置为0则为平面填充圆，等于area则为正常球体')
   );
 //           this.data.push(new StringValue('xyzangleStart', 'xyzangleStart', "0")
 //       .setTooltip('none')
 //   );
                this.data.push( new AttributeValue('speed', 'speed', 0,0)
        .setTooltip('none')
    );
                    this.data.push(new AttributeValue('amount', 'amount', 1,0)
        .setTooltip('none')
    );
  //                      this.data.push(new StringValue('xadd', 'xadd', "0")
  //      .setTooltip('none')
  //  );
  //                          this.data.push(new StringValue('yadd', 'yadd', "0")
  //      .setTooltip('none')
  //  );
  //                              this.data.push(new StringValue('zadd', 'zadd', "0")
  //      .setTooltip('none')
  //  );
            this.data.push(new ListValue('particleType', 'particleType', [ 'FLAME',
            'EXPLOSION_NORMAL',
            'EXPLOSION_LARGE',
            'EXPLOSION_HUGE' ,
            'FIREWORKS_SPARK',
            ...
            'NOTE',
            'PORTAL',
            'ENCHANTMENT_TABLE','FLAME','LAVA','FOOTSTEP','CLOUD','REDSTONE','SNOWBALL','SNOW_SHOVEL','SLIME','HEART','BARRIER','ITEM_CRACK','BLOCK_CRACK','BLOCK_DUST','WATER_DROP','ITEM_TAKE','MOB_APPEARANCE','DRAGON_BREATH','END_ROD','DAMAGE_INDICATOR','SWEEP_ATTACK','FALLING_DUST','TOTEM','SPIT' ], "FLAME")
        .setTooltip('none')
    );
    var reflectList = [ 'ITEM_CRACK','BLOCK_CRACK', 'BLOCK_DUST','FALLING_DUST','ITEM_TAKE'];
    this.data.push(new IntValue("材质ID", "dataType", 7).requireValue('particleType', reflectList)
        .setTooltip('数字ID')
    );
    this.data.push(new IntValue("材质Data", "dataId", 0).requireValue('particleType', reflectList)
    .setTooltip('')
);
        this.data.push(new AttributeValue('rgb_r', 'values_x', 0, 0)
        .setTooltip('The radius of the arrangement in blocks')
    );
        this.data.push(new AttributeValue('rgb_g', 'values_y', 0, 0)
        .setTooltip('The radius of the arrangement in blocks')
    );
        this.data.push(new AttributeValue('rgb_b', 'values_z', 0, 0)
        .setTooltip('The radius of the arrangement in blocks')
    );
}
extend('Jun_CircleParticle', 'Component');
function Jun_CircleParticle()
{
    this.super('Jun_CircleParticle', Type.MECHANIC, true);
    
    this.description = '生成一个平面的空心圆,使用颜色需要speed>0(speed为颜色的亮度)';
   ...
   this.data.push(new AttributeValue('平面旋转角度-xzangle', 'xzangle', 360,0)
       .setTooltip('none')
   );
//        this.data.push(new StringValue('xyzangle', 'xyzangle', "360")
//        .setTooltip('none')
//    );
       this.data.push(new AttributeValue('平面旋转开始角度-xzangleStart', 'xzangleStart', 0,0)
       .setTooltip('none')
   );
 //           this.data.push(new StringValue('xyzangleStart', 'xyzangleStart', "0")
 //       .setTooltip('none')
 ...
 //           this.data.push(new AttributeValue('values_area', 'values_area', 0, 0)
 //       .setTooltip('The radius of the arrangement in blocks')
 //   );
 //               this.data.push(new AttributeValue('values_speed', 'values_speed', 0, 0)
 //       .setTooltip('The radius of the arrangement in blocks')
 //   );
}
extend('Jun_LineParticle', 'Component');
function Jun_LineParticle()
{
    this.super('Jun_LineParticle', Type.MECHANIC, true);
    
    this.description = '以自身为中心向偏移点连线,使用颜色需要speed>0(speed为颜色的亮度),可在此效果上添加子效果,等于在偏移地点上生成的一个offset选择器';
    

    this.data.push(new AttributeValue('area', 'area', 1,0)
        .setTooltip('none')
    );
    ...
    this.data.push(new AttributeValue("forward", "x2", 0, 0)
    .setTooltip('none')
);
this.data.push(new AttributeValue("upward", "y2", 0, 0)
.setTooltip('none')
);
this.data.push(new AttributeValue("rightward", "z2", 0, 0)
.setTooltip('none')
);
this.data.push(new StringValue('lineToRemember', 'lineToRemember', "")
.setTooltip('连线至Remember')
);
// this.data.push(new AttributeValue("forward-b", "xadd", 0, 0)
// .setTooltip('none')
// );
// this.data.push(new AttributeValue("upward-b", "yadd", 0, 0)
// .setTooltip('none')
// );
// this.data.push(new AttributeValue("rightward-b", "zadd", 0, 0)
// .setTooltip('none')
// );

                this.data.push(new AttributeValue('speed', 'speed', 0,1)
        .setTooltip('none')
    );
                    this.data.push(new AttributeValue('amount', 'amount', 1,0)
        .setTooltip('none')
    );!
            this.data.push(new ListValue('HORIZONTAL', 'Yzero', [ 'True', 'False' ], 'True')
        .setTooltip('平行地面')
    );
    this.data.push(new AttributeValue('color_r', 'values_x', 0, 0)
        .setTooltip('The radius of the arrangement in blocks')
    );
        this.data.push(new AttributeValue('color_g', 'values_y', 0, 0)
        .setTooltip('The radius of the arrangement in blocks')
    );
        this.data.push(new AttributeValue('color_b', 'values_z', 0, 0)
        .setTooltip('The radius of the arrangement in blocks')
    );

}
extend('Germ', 'Component');
function Germ()
{
    this.super('GermEffect', Type.MECHANIC, false);
    
    this.description = '释放技能时生成萌芽特效';
    
    this.data.push(new StringValue('特效名称', 'effect', "defaultEffect")
        .setTooltip('萌芽特效在GermEffect.yml内的名称')
    );
    this.data.push(new ListValue('特效目标', 'target', [ 'SkillCaster', 'SkillTarget' ], 'SkillCaster')
        .setTooltip('萌芽特效生成时的基准目标')
    );
    this.data.push(new ListValue('绑定对象', 'bind', [ 'Entity', 'Location' ], 'Entity')
        .setTooltip('萌芽特效生成时的绑定对象类型')
    );
    this.data.push(new ListValue('附加效果', 'move', [ 'None', 'CasterToTarget', 'TargetToCaster'], 'None')
        .setTooltip('萌芽特效的额外附加动画效果')
    );
    this.data.push(new StringValue('移动时长', 'duration', "1000")
        .setTooltip('额外附加动画效果的移动时长')
    );
}
extend('LineClientEffect', 'Component');
function LineClientEffect()
{
    this.super('LineClientEffect', Type.MECHANIC, true);
    
    this.description = 'create a line particle between a to b';
    

    this.data.push(new StringValue('area', 'area', "1")
        .setTooltip('none')
    );
    ...
    this.data.push(new StringValue('forward-a', 'x2', "0")
        .setTooltip('none')
    );
        this.data.push(new StringValue('upward-a', 'y2', "0")
        .setTooltip('none')
    );
        this.data.push(new StringValue('rightward-a', 'z2', "0")
        .setTooltip('none')
    );

                this.data.push(new StringValue('speed', 'speed', "0")
        .setTooltip('none')
    );
                    this.data.push(new StringValue('amount', 'amount', "1")
        .setTooltip('none')
    );
                        this.data.push(new StringValue('forward-b', 'xadd', "0")
        .setTooltip('none')
    );
                            this.data.push(new StringValue('upward-b', 'yadd', "0")
        .setTooltip('none')
    );
                                this.data.push(new StringValue('rightward-b', 'zadd', "0")
        .setTooltip('none')
    );
            this.data.push(new ListValue('HORIZONTAL', 'Yzero', [ 'True', 'False' ], 'True')
        .setTooltip('平行地面')
...
extend('MechanicZhaomula', 'Component');
function MechanicZhaomula()
{
    this.super('Zhaomula', Type.MECHANIC, false);
    
    this.description = '允许使用数字、属性、变量进行简单的公式运算，可以指定一个小括号。注意：如果目标非施法者本身且为玩家，读取的属性值将是目标的；如果属性值为0，将修正为1（即参与运算的属性值至少为1）';
    
    this.data.push(new AttributeValue('Value 0', 'value-0', 'strength', 0)
        .setTooltip('（初始值）设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new ListValue('Operator 1', 'operator-1', [ '+', '-', '*', '/' ], '*')
        .setTooltip('选择运算符号，决定与下个数值如何运算。')
    );
    this.data.push(new AttributeValue('Value 1', 'value-1', 0, 0)
        .setTooltip('设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new ListValue('Operator 2', 'operator-2', [ '+', '-', '*', '/' ], '*')
        .setTooltip('选择运算符号，决定与下个数值如何运算。')
    );
    this.data.push(new AttributeValue('Value 2', 'value-2', 0, 0)
        .setTooltip('设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new ListValue('Operator 3', 'operator-3', [ '+', '-', '*', '/' ], '*')
        .setTooltip('选择运算符号，决定与下个数值如何运算。')
    );
    this.data.push(new AttributeValue('Value 3', 'value-3', 0, 0)
        .setTooltip('设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new ListValue('Operator 4', 'operator-4', [ '+', '-', '*', '/' ], '*')
        .setTooltip('选择运算符号，决定与下个数值如何运算。')
    );
    this.data.push(new AttributeValue('Value 4', 'value-4', 0, 0)
        .setTooltip('设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new ListValue('Operator 5', 'operator-5', [ '+', '-', '*', '/' ], '*')
        .setTooltip('选择运算符号，决定与下个数值如何运算。')
    );
    this.data.push(new AttributeValue('Value 5', 'value-5', 0, 0)
        .setTooltip('设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new ListValue('Operator 6', 'operator-6', [ '+', '-', '*', '/' ], '*')
        .setTooltip('选择运算符号，决定与下个数值如何运算。')
    );
    this.data.push(new AttributeValue('Value 6', 'value-6', 0, 0)
        .setTooltip('设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new ListValue('Operator 7', 'operator-7', [ '+', '-', '*', '/' ], '*')
        .setTooltip('选择运算符号，决定与下个数值如何运算。')
    );
    this.data.push(new AttributeValue('Value 7', 'value-7', 0, 0)
        .setTooltip('设置公式中单个运算项目的数值，可以填数字，属性名，变量名。使用"-"可以在三者中的任两者间指定一个范围，如 "3-5" / "5-阿巴阿巴" / "3-strength"')
    );
    this.data.push(new AttributeValue('Parenthesis', 'parenthesis', 0, 0)
        .setTooltip('如果两个框中的数值都有效，则优先运算指定范围的数值。')
    );
    this.data.push(new StringValue('Key', 'key', "key")
        .setTooltip('指定运算结果储存到哪个变量。')
    );
}
...
function addParticleOptions(component) {
    ...
    var reflectList = [ 'Smoke','Block Crack', 'Angry Villager', 'Bubble', 'Cloud', 'Crit', 'Damage Indicator', 'Depth Suspend', 'Dragon Breath', 'Drip Lava', 'Drip Water', 'Enchantment Table', 'End Rod', 'Explode', 'Firework Spark', 'Flame', 'Footstep', 'Happy Villager', 'Hear', 'Huge Explosion', 'Instant Spell', 'Large Explode', 'Large Smoke', 'Lava', 'Magic Crit', 'Mob Spell', 'Mob Spell Ambient', 'Note', 'Portal', 'Red Dust', 'Slime', 'Snowball Poof', 'Snow Shovel', 'Spell', 'Splash', 'Suspend', 'Sweep Attack', 'Totem', 'Town Aura', 'Water Drop', 'Water Wake', 'Witch Magic' ];
...
function addArmorEffect(component, optional){

    var opt = appendNone2;
    if (optional)
    {
        opt = appendOptionalArmor;
        
        component.data.push(new ListValue('盔甲架特效', '盔甲架特效', [ 'True', 'False' ], 'False')
            .setTooltip('使用盔甲架特效.')
        );
    }
    component.data.push(opt(new ListValue('小型盔甲架', '小型盔甲架', [ 'True', 'False' ], 'False')
    .setTooltip('The key to refer to the effect by. Only one effect of each key can be active at a time.')
));
component.data.push(opt(new ListValue('发光', '发光', [ 'True', 'False' ], 'False')
.setTooltip('The key to refer to the effect by. Only one effect of each key can be active at a time.')
));
component.data.push(opt(new IntValue('盔甲架的头部物品数字ID', '盔甲架的头部物品数字ID', 276)
.setTooltip('Number of ticks between playing particles.')
));
component.data.push(opt(new StringValue('盔甲架的头部物品LORE', '盔甲架的头部物品LORE', '小脑斧')
.setTooltip('此项配合材质包制作特效')
));
component.data.push(opt(new StringValue('盔甲架remember', '盔甲架remember', 'none')
.setTooltip('将此盔甲架记录至remember')
));
component.data.push(opt(new DoubleValue('头部X旋转角度', '头部X旋转角度', 0)
.setTooltip('Number of ticks between playing particles.')
));
component.data.push(opt(new DoubleValue('头部Y旋转角度', '头部Y旋转角度', 0)
.setTooltip('Number of ticks between playing particles.')
));
component.data.push(opt(new DoubleValue('头部Z旋转角度', '头部Z旋转角度', 0)
.setTooltip('Number of ticks between playing particles.')
));
component.data.push(opt(new DoubleValue('头部X旋转角度变化', '头部X旋转角度变化', 0)
.setTooltip('Number of ticks between playing particles.')
));
component.data.push(opt(new DoubleValue('头部Y旋转角度变化', '头部Y旋转角度变化', 0)
.setTooltip('Number of ticks between playing particles.')
));
component.data.push(opt(new DoubleValue('头部Z旋转角度变化', '头部Z旋转角度变化', 0)
.setTooltip('Number of ticks between playing particles.')
));

}
function addModEffect(component, optional){

    var opt = appendNone2;
    if (optional)
    {
        opt = appendOptionalMod;
        component.data.push(new ListValue('使用MOD特效', '使用MOD特效', [ 'True', 'False' ], 'False')
            .setTooltip('使用MOD特效,dz是时间,dy是范围，转速=speed.amout=密度大于100时特效连接至施法者')
        );
    }
    component.data.push(opt(new ListValue('MOD特效ID', 'MOD特效ID', 
    [ 
        'null', 
        'electromagnetism', 
        'FLASH', 
    ...
    .setTooltip('MOD中的特效,zhen震动可以被time影响')
    ))
    ;
    component.data.push(opt(new StringValue('旋转', '旋转', 1)
        .setTooltip('')
    ));
    component.data.push(opt(new StringValue('转速', '转速', 1)
    .setTooltip('')
));
component.data.push(opt(new StringValue('转速增加', '转速增加', 0.01)
.setTooltip('')
));
component.data.push(opt(new StringValue('rgb', 'rgb', '255,255,255')
.setTooltip('name.')
));
component.data.push(opt(new StringValue('rgb2', 'rgb2', '255,255,255')
.setTooltip('name.')
));
component.data.push(opt(new StringValue('混色范围', '混色范围', '0.1')
.setTooltip('name.')
));
component.data.push(opt(new StringValue('向量Remember', '向量Remember', 'null')
.setTooltip('此项粒子向着remember方向飞.')
));
component.data.push(opt(new StringValue('替换cast', '替换cast', 'null')
.setTooltip('连线用.')
));
component.data.push(opt(new StringValue('替换targ', '替换targ', 'null')
.setTooltip('连线用.')
));
component.data.push(opt(new StringValue('混色透明度', '混色透明度', '0.6')
.setTooltip('name.')
));
component.data.push(opt(new StringValue('相对上升高度', '相对上升高度', '0')
.setTooltip('amount设置为100则使粒子跟随目标，此项设置相对高度')
));
component.data.push(opt(new StringValue('粒子跟随', '附加值1', '0')
.setTooltip('用于magicfire 0为最正常的粒子 带颜色 2为不带颜色 1250颜色+向量 100跟随实体+颜色+向量 200跟随实体+向量 其它else')
));
component.data.push(opt(new StringValue('随机速度', '附加值2', '0')
.setTooltip('增加随机速度')
));
component.data.push(opt(new StringValue('粒子范围', '附加值3', '0')
.setTooltip('amount设置为100则使粒子跟随目标，此项设置相对高度')
));
component.data.push(opt(new StringValue('渐变颜色r', '附加值4', '0')
.setTooltip('amount设置为100则使粒子跟随目标，此项设置相对高度')
));
component.data.push(opt(new StringValue('渐变颜色g', '附加值5', '0')
.setTooltip('amount设置为100则使粒子跟随目标，此项设置相对高度')
));
component.data.push(opt(new StringValue('渐变颜色b', '附加值6', '0')
.setTooltip('amount设置为100则使粒子跟随目标，此项设置相对高度')
));
}
function addEntityEffect(component, optional){

    var opt = appendNone2;
    if (optional)
    {
        opt = addendEntityEffect;
        
        component.data.push(new ListValue('使用实体特效', '使用实体特效', [ 'True', 'False' ], 'False')
            .setTooltip('使用实体特效.')
        );
    }
component.data.push(opt(new ListValue('实体发光', '实体发光', [ 'True', 'False' ], 'False')
.setTooltip('qwq')
));
component.data.push(opt(new ListValue('实体着火', '实体着火', [ '1000', '0' ], '0')
.setTooltip('以tick计时')
));
component.data.push(opt(new StringValue('实体名称', '实体名称', 'none')
.setTooltip('name.')
));
component.data.push(opt(new ListValue('实体ID', '实体ID', 
[ 
    'Zombie Villager', 
    'Zombie Horse',
    'Zombie', 
...
function appendOptionalArmor(value)
{
    value.requireValue('盔甲架特效', [ 'True' ]);
    return value;
}
function appendOptionalMod(value)
{
    value.requireValue('使用MOD特效', [ 'True' ]);
    return value;
}
function addendEntityEffect(value)
{
    value.requireValue('使用实体特效', [ 'True' ]);
    return value;
}
...
function appendNone2(value)
{
    return value;
}
function appendNone3(value)
{
    return value;
}
